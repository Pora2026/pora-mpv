...
def iter_month_labels(d1: date, d2: date) -> list[str]:
    """Lista explícita de meses YYYY-MM entre d1 y d2 (inclusive)."""
...
class BusinessDay(db.Model):
    ...
    # NUEVO: Ganancia real cargada manualmente (dueño)
...
def ensure_schema():
    """
    Mini-migración sin Alembic:
    - Agrega business_days.real_profit si no existe (SQLite/Postgres).
    """
    ...
    try:
        ...
        # Si falla por permisos/lock/etc, no rompemos el arranque.
        db.session.rollback()
...
def home():
    ...
    <div class="grid3">
      <div class="card">
        <h3>Panel Central</h3>
        <p class="muted">Dashboard + gráficos + alertas + control de ganancia real.</p>
...
# ----------------------------
# Panel Central (antes: Gestión Financiera)
# ----------------------------
@app.get("/finanzas")
@login_required
def dashboard_finanzas():
    ...
    # Torta del período (valores en $). Si hay pérdida, mostramos "Pérdida" abs.
    if income > 0:
    ...
    # ---------------------------------------------------------
    # NUEVO BLOQUE: Ganancia Calculada vs Real (DIARIO)
    # - Listado por día en el rango (sin domingos), incluso si falta el día (0)
    # - Real editable y persistente (BusinessDay.real_profit)
    # - Estado OK / NO OK
    # - Diferencia en rojo
    # - Gráfico comparativo (Calc vs Real)
    # - Tabla con desplegable (3 por defecto)
    # ---------------------------------------------------------
    ...
    for d in all_days:
        ...
        ok = (diff == 0) if (diff is not None) else False
    ...
    # Payload chart: si real es None => null (gap)
    ...
    # Tabla: 3 por defecto, resto en details
    ...
    def _cmp_tr(r):
        if r["diff"] is None:
            ...
            diff_cls = "neg" if r["diff"] != 0 else ""
            diff_html = f"<span class='{diff_cls}'>{ars(r['diff'])}</span>"
            status_html = "<span class='pill ok'>OK</span>" if r["diff"] == 0 else "<span class='pill bad'>NO OK</span>"

        real_val = "" if r["real"] is None else str(float(r["real"]))
        # input manual + submit por fila (simple y robusto)
        form = f"""
        <form method="post" action="/finanzas/real_profit/save" class="inline" style="margin:0; justify-content:flex-end;">
          <input type="hidden" name="from" value="{from_str}" />
          <input type="hidden" name="to" value="{to_str}" />
    ...
    <!-- NUEVO: Control Ganancia Calculada vs Real (DIARIO) -->
    <div class="card">
    ...
    <script>
      ...
      // NUEVO: Chart comparativo Calc vs Real
...
@app.post("/finanzas/real_profit/save")
@login_required
def save_real_profit():
    """
    Guarda BusinessDay.real_profit para una fecha.
    Si el día no existe, lo crea (draft) y deja real_profit seteado.
    Redirige de nuevo al rango del Panel Central.
    """
    ...
    if is_sunday(d):
        flash("Domingo: no se trabaja. No se guarda.", "error")
        return redirect(url_for("dashboard_finanzas", **({"from": from_str, "to": to_str} if from_str and to_str else {})))

    # Permite limpiar el valor (vacío => None)
    real_profit = None
    if v != "":
        try:
            real_profit = safe_float(v)
        except Exception:
            flash("Ganancia real inválida.", "error")
            return redirect(url_for("dashboard_finanzas", **({"from": from_str, "to": to_str} if from_str and to_str else {})))
    ...
    flash("Ganancia real guardada.", "ok")
    return redirect(url_for("dashboard_finanzas", **({"from": from_str, "to": to_str} if from_str and to_str else {})))
...
def io_dashboard():
    ...
    # ✅ FIX PUNTO 1: labels de meses se generan explícitamente desde rango d1→d2
    all_months = iter_month_labels(d1, d2)
    trace_labels = all_months
    ...
    <div class="card">
      ...
      <p class="muted" style="margin-top:10px;">
        Muestra hasta 6 categorías más “pesadas” del rango, mes a mes. Incluye meses sin datos (0).
      </p>
    </div>
...
def list_days():
    ...
    for d in days:
        ...
        # ✅ PUNTO 4: Estado del día (Bueno/Regular/Malo) según margen
